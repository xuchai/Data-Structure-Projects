...
class DistancePixel {
 public:
  DistancePixel() : x(-1),y(-1),distance(std::numeric_limits<double>::max()), is_fixed(false) {}
  //a constructor with formal parameter******************************
  DistancePixel(int x_loc, int y_loc, double dis, bool fixed): x(x_loc), y(y_loc), distance(dis), is_fixed(fixed) {};
  //***********************************************************************
  // accessor
  int getX() const { return x; }
  int getY() const { return y; }
  bool get_fixed() const {return is_fixed;}
  double getValue() const { return distance; }
  // modifier
  void setX(int _x) { x = _x; }
  void setY(int _y) { y = _y; }
  void set_fixed(bool new_fixed_property) {is_fixed = new_fixed_property;}
 ...
 private:
  ...
  double distance;
  // the boolean value whether it is fixed
  bool is_fixed;
};
