...
void student_tests() {
  ...
  //************************ Test corner case QuadTree  ************************

  std::cout << "TEST CASE 1: A QuadTree whose children are only linked to the left_most node" << std::endl;

  
  simple_points.push_back(std::make_pair(Point<int>(40,40), 'A'));
  simple_points.push_back(std::make_pair(Point<int>(35,35), 'B'));
  simple_points.push_back(std::make_pair(Point<int>(30,32), 'C'));
  simple_points.push_back(std::make_pair(Point<int>(28,30), 'D'));
  simple_points.push_back(std::make_pair(Point<int>(26,26), 'E'));
  simple_points.push_back(std::make_pair(Point<int>(25,24), 'F'));
  simple_points.push_back(std::make_pair(Point<int>(24,21), 'G'));
  simple_points.push_back(std::make_pair(Point<int>(17,19), 'H'));
  simple_points.push_back(std::make_pair(Point<int>(15,16), 'I'));
  simple_points.push_back(std::make_pair(Point<int>(12,14), 'J'));
  simple_points.push_back(std::make_pair(Point<int>(10,13), 'K'));
  simple_points.push_back(std::make_pair(Point<int>(9,10), 'L'));
  simple_points.push_back(std::make_pair(Point<int>(8,7), 'M'));
  
{
  ...
  std::cout << "the height of the tree is " << simple.height()<< std::endl;

  //**********************    Test find() function      ***************************
  int x=15,y=13;
  QuadTree<int,char>::iterator itr_1 = simple.find(x,y);
  if(itr_1==simple.end()){
    std::cout<<" No node at ("<<x<<", "<<y<<" )"<<std::endl;
  }
  else{
  assert (itr_1 != simple.end());
  std::cout <<" the label we found based on its coordinates is " <<itr_1.getLabel()
            <<" at (" <<(*itr_1).x<< ", " <<(*itr_1).y<< ")" <<std::endl ;

  std::cout<<"The height of this  tree is "<<simple.height()<<std::endl;
  //************************  Tests Iterator classes      *************************
  ...
  // note: this is a pre-order traversal, the same order as the 'sideways' tree above!!
  std::cout << "\nA depth-first traversal of the simple tree (should match sideways output!):" << std::endl;
  QuadTree<int,char>::iterator df_itr_1 = simple.begin();
  for (int i = 0; i < simple.size(); i++) {
    assert (df_itr_1 != simple.end());
    // ***********************  Tests getDepth() function  *************************
    int depth = df_itr_1.getDepth();
    // use the depth to indent the output (& match the sideways tree output above)
    std::cout << std::string(depth*2,' ') << df_itr_1.getLabel() << " " << *df_itr_1 << std::endl;
    // ***********************  Tests the pre-increment operator++   ****************
    ++df_itr_1;  
  }
  // after all increments, we better be at the end!
  assert (df_itr_1 == simple.end());
  ...
  // using the breadth-first iterator to traverse the data by level
  std::cout << "\nA breadth first traversal of the simple tree:";
  QuadTree<int,char>::bf_iterator bf_itr_1 = simple.bf_begin();
  int level_1 = -1;
  for (int i = 0; i < simple.size(); i++) {
    assert (bf_itr_1 != simple.bf_end());
    int depth = bf_itr_1.getDepth();
    if (level_1 != depth) {
      level_1 = depth;
      // starting a new level!
      std::cout << std::endl << "   level " << level_1 << ":";
    }
    // print out this data point
    std::cout << " " << bf_itr_1.getLabel() << *bf_itr_1;
    ++bf_itr_1;
  }
  // after all increments, we better be at the end!
  assert (bf_itr_1 == simple.bf_end());
  ...
  std::cout << "TEST CASE 2: A QuadTree whose children are in the same axis" << std::endl;

  simple_points.push_back(std::make_pair(Point<int>(3,13), 'N'));
  simple_points.push_back(std::make_pair(Point<int>(16,13), 'O'));
  simple_points.push_back(std::make_pair(Point<int>(4,13), 'P'));
  simple_points.push_back(std::make_pair(Point<int>(15,13), 'Q'));
  simple_points.push_back(std::make_pair(Point<int>(25,13), 'R'));
  simple_points.push_back(std::make_pair(Point<int>(37,13), 'S'));
  simple_points.push_back(std::make_pair(Point<int>(24,13), 'T'));
  simple_points.push_back(std::make_pair(Point<int>(36,13), 'U'));
...

{
  ...
  //**********************    Test find() function      ***************************
  int x=15,y=13;
  QuadTree<int,char>::iterator itr_2 = simple.find(x,y);
  if(itr_2==simple.end()){
    std::cout<<" No node at ("<<x<<", "<<y<<" )"<<std::endl;
  }
  else{
  assert (itr_2 != simple.end());
  std::cout <<" the label we found based on its coordinates is " <<itr_2.getLabel()
            <<" at (" <<(*itr_2).x<< ", " <<(*itr_2).y<< ")" <<std::endl ;
  ...
  // note: this is a pre-order traversal, the same order as the 'sideways' tree above!!
  std::cout << "\nA depth-first traversal of the simple tree (should match sideways output!):" << std::endl;
  QuadTree<int,char>::iterator df_itr_2 = simple.begin();
  for (int i = 0; i < simple.size(); i++) {
    if (df_itr_2 != simple.end()){
    // ***********************  Tests getDepth() function  *************************
    int depth = df_itr_2.getDepth();
    // use the depth to indent the output (& match the sideways tree output above)
    std::cout << std::string(depth*2,' ') << df_itr_2.getLabel() << " " << *df_itr_2 << std::endl;}
    // ***********************  Tests the pre-increment operator++   ****************
    ++df_itr_2;  
  }
  // after all increments, we better be at the end!
  assert (df_itr_2 == simple.end());


  // --------------------------------------------------------
  // using the breadth-first iterator to traverse the data by level
  std::cout << "\nA breadth first traversal of the simple tree:";
  QuadTree<int,char>::bf_iterator bf_itr_2 = simple.bf_begin();
  int level_1 = -1;
  for (int i = 0; i < simple.size(); i++) {
    if (bf_itr_2 != simple.bf_end()){
    int depth = bf_itr_2.getDepth();
    if (level_1 != depth) {
      level_1 = depth;
      // starting a new level!
      std::cout << std::endl << "   level " << level_1 << ":";
    ...
    // print out this data point
    std::cout << " " << bf_itr_2.getLabel() << *bf_itr_2;}
    ++bf_itr_2;
  }
  // after all increments, we better be at the end!
  assert (bf_itr_2 == simple.bf_end());
  std::cout << std::endl;
 ...
 std::cout << "TEST CASE 3: A QuadTree who has multiple nodes with exactly same coordinates but different labels" << std::endl;
   simple_points.push_back(std::make_pair(Point<int>(16,13), 'N'));
  simple_points.push_back(std::make_pair(Point<int>(16,13), 'O'));
  simple_points.push_back(std::make_pair(Point<int>(4,13), 'P'));
  simple_points.push_back(std::make_pair(Point<int>(16,13), 'Q'));
  simple_points.push_back(std::make_pair(Point<int>(25,13), 'R'));
  simple_points.push_back(std::make_pair(Point<int>(37,13), 'S'));
  simple_points.push_back(std::make_pair(Point<int>(24,13), 'T'));
  simple_points.push_back(std::make_pair(Point<int>(36,13), 'U'));

{
 ...
 std::cout << "TEST CASE 4: An empty QuadTree" << std::endl;
...

{
  ...
  //**********************    Test find() function      ***************************
  int x=15,y=13;
  QuadTree<int,char>::iterator itr = simple.find(x,y);
  if(itr==simple.end()){
    std::cout<<" No node at ("<<x<<", "<<y<<" )"<<std::endl;
  }
  else{
  assert (itr != simple.end());
  std::cout <<" the label we found based on its coordinates is" <<itr.getLabel()
            <<" at (" <<(*itr).x<< ", " <<(*itr).y<< ")" <<std::endl ;

  std::cout << "\na 'sideways' printing of the finished tree structure:" << std::endl;
 }
  //************************  Tests Iterator classes      *************************
  std::cout<<"The height of this empty tree is "<<simple.height()<<std::endl;
  simple.print_sideways();

  // use the primary (depth-first) iterator to traverse the tree structure
  // note: this is a pre-order traversal, the same order as the 'sideways' tree above!!
  std::cout << "Testing depth-first increments" << std::endl;
  QuadTree<int,char>::iterator df_itr = simple.begin();
  for (int i = 0; i < simple.size(); i++) {
    // ***********************  Tests the pre-increment operator++   ****************
    ++df_itr;  
  }
  // the iterator should points to a NULL node
  assert (df_itr == simple.end());


  // --------------------------------------------------------
  // using the breadth-first iterator to traverse the data by level
  std::cout << "Testing breadth-first increments"<<std::endl;
  QuadTree<int,char>::bf_iterator bf_itr = simple.bf_begin();
  int level = -1;
  for (int i = 0; i < simple.size(); i++) {
    ++bf_itr;
